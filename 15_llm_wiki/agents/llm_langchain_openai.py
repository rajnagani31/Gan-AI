from adapter.langchain_openai import LangChainOpenAIAdapter
from tools import get_weather


adapter = LangChainOpenAIAdapter(model="gpt-4.1-mini")
llm_with_tools = adapter.bind_tools([get_weather])

for chunk in llm_with_tools.stream("HI"):
    print(chunk, end="")
...
def stream_response(messages: list) -> AIMessage:
    """Print one streamed OpenAI turn and return its complete tool-call message."""
    chunks = []
